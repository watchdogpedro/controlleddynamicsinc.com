...
const podcastSegments: PodcastSegment[] = [
 ...
 {
 number: 4,
 title: 'friction-based Limitations: Static vs. Dynamic',
 overview: 'Traditional friction-based systems work in static applications but have fundamental weaknesses: all contact points are parallel, providing frictional force in only a single plane.',
 fullContent: 'Let\'s look at traditional systems. Numerous companies manufacture modular aluminum framing systems, and they nearly all employ the basic friction-based fastening system. This system relies on using an oversized fastener tightened to a high torque. They do this in an effort to keep assembled components from moving when external forces are applied. In static situations, this assembly might never experience an issue. However, this system\'s points of contact are all parallel, meaning the frictional force is only in a single direction or plane.'
 },
 {
 number: 5,
 title: 'friction-based Failure in Motion',
 overview: 'In dynamic situations with movement or vibration, friction-based fastening creates pivot points that allow slippage, causing misalignment and loosening fasteners.',
 fullContent: 'The weakness of the friction-based system becomes evident in dynamic situations. When exposed to movement, the fastening system creates pivot points. These pivots allow for movement or slippage between the components. This movement causes misalignment and ultimately loosens the fastener, compromising the assembly. The friction-based system creates significant headaches and heartache when subject to anything that\'s vibrating or creating resistance to being stagnant.'
 },
 {
 number: 6,
 title: 'Distortion and Compounding Misalignment',
 overview: 'Oversized friction-based fasteners torqued to specification measurably distort the profile outward, compounding misalignment issues caused by dynamic forces.',
 fullContent: 'Another critical issue with the existing friction-based technology is distortion. The use of an oversized fastener, torqued to specification, can measurably distort the profile. This distortion pushes the profile outward well beyond the extrusion tolerances. This outward pressure actually compounds the misalignment caused by dynamic forces. This demonstrates why relying solely on high torque and friction in a single plane is insufficient for modern structural needs.'
 },
 {
 number: 7,
 ...
 fullContent: 'The key to correcting the issues inherent in existing friction-based fastening systems lies in eliminating all pivot points. Control Dynamics achieves this by creating a mechanical lock. The AngleLock system ensures you don\'t have to worry about bolts coming loose or things coming out of alignment. It is a totally different type of structure built for reliability.'
 },
 {
 number: 8,
...
export default function TechnicalSpecificationsPage() {
 ...
 Think of the AngleLock system versus the friction-based system as the difference between
 using <strong className="text-[#C9A227]">glue tape versus interlocking bricks</strong>.
 </p>
 <p className="text-white/90 leading-relaxed mb-4">
 friction-based systems rely on friction—like glue tape—which holds well statically, but
